...
function cmeDataViewer_MA_multipleMontages(data, varargin)
...
%===============================================================================
    ...
    function montageButton_Callback(varargin)
        ...
        nbMontages = input('How many montages do you want?');
        
        totalNbTracks = length(tracks);
        
        fprintf('Generating montage...');
        
        trackMontageOrder = randperm(totalNbTracks, nbMontages)
        
%         figure(2); clf;
        
        %         nn = 1;
        for n = trackMontageOrder
            
            if nbMontages ~=1
                trackIndex.current = n;
            end
                     disp(trackIndex.current)
            % Creates a montage based on the master track
            %         if ~isempty(trackIndex.current)
            
            %             figure(2);
            %             subplot(nbMontages, 1, nn); hold on;
            ...
            %                 plotTrackMontageMultiples(nn, tracks(trackIndex.current), istack, xa, ya, 'Labels', data.markers,...
            if ~isempty(tracks(n).x)
        ...
        if ~isempty(tracks)
            ...
            fpath = [data.source 'Figures' filesep 'track_' num2str(trackIndex.loaded(trackIndex.current)) '_montage.eps'];
                plotTrackMontage(tracks(trackIndex.current), tstack, xa, ya, 'Labels', data.markers,...
                    ...
                    'ShowMarkers', get(montageMarkerCheckbox, 'Value')==1,...
                    'ShowDetection', get(montageDetectionCheckbox, 'Value')==1);
%                 % save montage as png too
%              fpath2 = [data.source 'Figures' filesep 'track__' num2str(trackIndex.loaded(trackIndex.current)) '_montage.png'];
%                 plotTrackMontage(tracks(trackIndex.current), tstack, xa, ya, 'Labels', data.markers,...
%                     'Visible', 'off', 'epsPath', fpath2,...
%                     'ShowMarkers', get(montageMarkerCheckbox, 'Value')==1,...
%                     'ShowDetection', get(montageDetectionCheckbox, 'Value')==1);    
        end
