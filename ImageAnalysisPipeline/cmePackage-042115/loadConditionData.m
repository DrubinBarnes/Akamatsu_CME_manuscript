...
ip.addParameter('Parameters', [1.49 100 6.45e-6], @(x) numel(x)==3);
...
ip.addParameter('LoadThrough', false,  @isslogical);
ip.parse(varargin{:});
...
% make a list of files or folders to ignore if they are in the directory.
% these come from previous versions of the tracking. MA 2018

analysisNames = {'montage', 'movie', 'associated_tracks_ref_cor.mat', 'manually_cleaned_track_list.mat', 'manually_cleaned_track_list.txt', 'cleaned_track_list.mat','cleaned_track_list.txt'};

% Load/determine channel names

nChNotEmpty=0;
if isempty(chNames)
    subfolders = dir(cellPath{1});
    subfolders = subfolders(3:end);
    nCh = numel(subfolders);
    chNames = cell(1, nCh);
    markers = cell(1, nCh);
    for f = 1:nCh
        
        % remove from the "chNames" list any folders that match the above
        % protected names (MA 2018)
        
        isTrackedFolder=strfind(analysisNames, subfolders(f).name);
        if ~any(vertcat(isTrackedFolder{:}))
        
            chNames{f} = subfolders(f).name;
            markers{f} = subfolders(f).name;
            nChNotEmpty=nChNotEmpty+1;
        end
        
    end
    
end
% re-calcluate nubmer channels.
nCh=nChNotEmpty;
