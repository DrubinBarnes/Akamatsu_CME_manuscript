...
% save masks
numChannels = length(data(1).markers);

for i = 1:numel(data)
    if ~hasStoredMask(i)
        for j = 1:numChannels
            [~,~] = mkdir([data(i).channels{j} 'Detection']);
            maskPath = [data(i).channels{j} 'Detection' filesep 'cellmask.tif'];
            imwrite(uint8(mask{i}), maskPath, 'tif', 'compression' , 'lzw');
            
            projPath = [data(i).channels{j} 'Detection' filesep 'maxproj.tif'];
