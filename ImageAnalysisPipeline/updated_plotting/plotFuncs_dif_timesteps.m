...
function [field_positions, lifetime_associated]= plotFuncs_dif_timesteps(S)
...
%% check if time interval was set to zero, then activate resampling all tracks to 1 s for aligning tracks of different time sampling.

if interval==0
    disp('resampling to 1 s interval for all the movies')
    resampleTimeInterval=1;
    
else
    resampleTimeInterval=0;
end

%% Name of the results files
histogram_file_name = fullfile(data_folder, 'histogram_pool_data.txt');

%% Just to get the final number of max time points
...
%                     if resampleTimeInterval
%                         % read in based on _2s etc as end of folder 
%                         
%                         interval = str2num(track_folder_name(end-1));
%                         fprintf('folder %s: interval %i s', track_folder_name, interval)
% %                         manual input
% %                         interval = input(['time step (s) for this field? ' track_folder_name ': '] );
...
%                     end
                    found = true;
                    track_data_temp = load(fullfile(track_folder_name, 'associated_tracks_ref_cor.mat'));
%                     tp_max_temp = track_data.analysis_info.tp_max;
                    
                    % make tp_max larger if it's already been input from
                    % another folder.

                    if tp_max

                        tp_max_cur = track_data_temp.analysis_info.tp_max;

                        if tp_max_cur>tp_max
                            tp_max=tp_max_cur
                        end
                    else
                        tp_max = track_data_temp.analysis_info.tp_max
                    end
                end
            end
        end
...
%                 tp_max should be max for all files: time step * nb frames
%                 
%                 % make tp_max larger if it's already been input from
%                 % another folder.
%                 
%                 if tp_max
%                                         
%                     tp_max_cur = track_data.analysis_info.tp_max;
%                     
%                     if tp_max_cur>tp_max
%                         tp_max=tp_max_cur
%                     end
                    ...
                    if align_ref
                        for i = 1:ref_cor_num
                            ...
                            while(smoothened_ref(n) < (max_value / 2))
                             ...
                             % SKIP this track if you don't have data for
                            % 50% dropoff (track ended too early)
                           
                            if n == tp_max
                                disp(['Skipped track ' num2str(i) ' because 50% dropoff intensity is not contained in the track.'])
                                continue
                        ...
                        % align cor
                        for i = 1:ref_cor_num
%                             disp(['working on track ' num2str(i)])
...
%% Histogram of lifetime, show only the associated tracks only.
    ...
    function [] = plotAlignCorrMax
        if num_associated > 0
            ...
            [h, N_sample] = drawAveragePlots(ref_aligned2cormax, cor_aligned2cormax, track_data.analysis_info, 'associated', lifetime_associated, h, tp_max, intervals, betweenDatasets);
            figure(h)
    ...
    function [] = plotAlignRefMax
        if num_associated > 0
            ...
            [h, N_sample] = drawAveragePlots(ref_aligned2refmax, cor_aligned2refmax, track_data.analysis_info, 'associated', lifetime_associated, h, tp_max, intervals, betweenDatasets);
            figure(h)
    ...
    function [] = plotHalfDropoff
        if num_associated > 0
            h = figure('OuterPosition', [fig_width, scrsz(4)-fig_height*2, fig_width,   fig_height]);
            [h, N_sample] = drawAveragePlots(ref_aligned_half, cor_aligned_half, track_data.analysis_info, 'associated', lifetime_associated, h, tp_max, intervals, betweenDatasets);
            figure(h)
    ...
    function [] = plotAlignRefStart
        if num_associated > 0
            ...
            [h, N_sample] = drawAveragePlots(ref_aligned2ref_begin, cor_aligned2ref_begin, track_data.analysis_info, 'associated', lifetime_associated, h, tp_max, intervals, betweenDatasets);
            figure(h)
    ...
    function [] = plotAlignRefEnd
        if num_associated > 0
            ...
            [h, N_sample] = drawAveragePlots(ref_aligned2ref_end, cor_aligned2ref_end, track_data.analysis_info, 'associated', lifetime_associated, h, tp_max, intervals, betweenDatasets);
            figure(h)
    ...
    function [] = plotAlignCorrStart
        if num_associated > 0
            ...
            [h, N_sample] = drawAveragePlots(ref_aligned2cor_begin, cor_aligned2cor_begin, track_data.analysis_info, 'associated', lifetime_associated, h, tp_max, intervals, betweenDatasets);
            figure(h)
    ...
    function [] = plotAlignCorrDisappear
        if num_associated > 0
            ...
            [h, N_sample] = drawAveragePlots(ref_aligned2cor_end, cor_aligned2cor_end, track_data.analysis_info, 'associated', lifetime_associated, h, tp_max, intervals, betweenDatasets);
            figure(h)
...
%% Plot unassociated reference tracks
    function [] = plotUnassociatedRef
        if num_ref_only>0
            ...
            [h, N_sample] = drawAveragePlots(ref_only_ref_aligned2ref_begin, ref_only_cor_aligned2ref_begin, track_data.analysis_info, 'ref', lifetime_ref_only, h, tp_max, intervals, betweenDatasets);
            figure(h)
            ...
            [h, N_sample] = drawAveragePlots(ref_only_ref_aligned2ref_end, ref_only_cor_aligned2ref_end, track_data.analysis_info, 'ref', lifetime_ref_only, h, tp_max, intervals, betweenDatasets);
            figure(h)
            ...
            [h, N_sample] = drawAveragePlots(ref_only_ref_aligned2refmax, ref_only_cor_aligned2refmax, track_data.analysis_info, 'ref', lifetime_ref_only, h, tp_max, intervals, betweenDatasets);
            figure(h)
...
%% Plot unassociated corresponding tracks
    function [] = plotUnassociatedCorr
        if num_cor_only>0
            ...
            [h, N_sample] = drawAveragePlots(cor_only_ref_aligned2cor_begin, cor_only_cor_aligned2cor_begin, track_data.analysis_info, 'cor', lifetime_cor_only, h, tp_max, intervals, betweenDatasets);
            figure(h)
            ...
            [h, N_sample] = drawAveragePlots(cor_only_ref_aligned2cor_end, cor_only_cor_aligned2cor_end, track_data.analysis_info, 'cor', lifetime_cor_only, h, tp_max, intervals, betweenDatasets);
            figure(h)
            ...
            [h, N_sample] = drawAveragePlots(cor_only_ref_aligned2cormax, cor_only_cor_aligned2cormax, track_data.analysis_info, 'cor', lifetime_cor_only, h, tp_max, intervals, betweenDatasets);
            figure(h)
...
function [h, N_sample] = drawAveragePlots(ref_profile, cor_profile, analysis_info, category, track_lifetimes, h, tp_max, intervals, betweenDatasets)
...
% account for different time intervals.

interval = analysis_info.interval;

% choose whether to rescale the intensity between datasets such that peak
% intensity within a dataset is 1000 AU
rescale_intensity_between_datasets = true


% interpolate cor_profile and ref_profile to 1 s intervals

% mark which tracks are from which dataset (first and last elements are
% first and last tracks)

datasetSeparations= [0, betweenDatasets];

%  if 'intevals' has not been defined for each cell, then just set it to be
%  whatever it is for the current cell (interval)
...
% ref_profile_shift= NaN(size(ref_profile));
% cor_profile_shift= NaN(size(cor_profile));

ref_profile_interp_rescaled = NaN(size(1:tp_max*maxInterval*2));
cor_profile_interp_rescaled = NaN(size(1:tp_max*maxInterval*2));

% each dataset
for j = 1:length(datasetSeparations)-1
    
    if intervals
        interval=intervals(j);
    end
    for i = datasetSeparations(j)+1:datasetSeparations(j+1)
        
        % rescale with interpolation to 1 s intervals.
        ref_profile_interp(i,:)=interp1(interval:interval:tp_max*interval*2,ref_profile(i,:),1:tp_max*maxInterval*2);
        cor_profile_interp(i,:)=interp1(interval:interval:tp_max*interval*2,cor_profile(i,:),1:tp_max*maxInterval*2);
    end
    
    % rescale intensity such that peak intensity is 1000 AU (within a
    % dataset). This is a way to compare datasets with different imaging
    % conditions.
    
%     for each dataset (the i values dictated by datasetSeparations)
    if rescale_intensity_between_datasets
        
        % last set of indecies for dataset should be skipped
        if datasetSeparations(j+1)<datasetSeparations(j)
            continue
        end
        
        cur_intensity_ref = ref_profile_interp(datasetSeparations(j)+1:datasetSeparations(j+1),:);
        cur_intensity_cor = cor_profile_interp(datasetSeparations(j)+1:datasetSeparations(j+1),:);
            
        % mean across time (of aligned tracks)

        cur_mean_intensity_ref = nanmean(cur_intensity_ref);
        cur_mean_intensity_cor = nanmean(cur_intensity_cor);
            
        % peak value (of the mean across time)

        cur_peak_intensity_ref = nanmax(cur_mean_intensity_ref);
        cur_peak_intensity_cor = nanmax(cur_mean_intensity_cor);
        
         % divide intensities by peak value. multiply by 1000: define 1000 as peak
         % AU value (for a time-averaged value in an expt)
        
        cur_ref_profile_interp_rescaled = cur_intensity_ref/cur_peak_intensity_ref*1000;
        cur_cor_profile_interp_rescaled = cur_intensity_cor/cur_peak_intensity_cor*1000;
                
        % add back into a variable common to all datasets
        
        ref_profile_interp_rescaled(datasetSeparations(j)+1:datasetSeparations(j+1),:) = cur_ref_profile_interp_rescaled;
        cor_profile_interp_rescaled(datasetSeparations(j)+1:datasetSeparations(j+1),:) = cur_cor_profile_interp_rescaled;
    end
end

if rescale_intensity_between_datasets
    ref_profile_interp = ref_profile_interp_rescaled;
    cor_profile_interp = cor_profile_interp_rescaled;
end

    % shift based on interval
    ref_profile_shift= NaN(size(ref_profile_interp));
    cor_profile_shift= NaN(size(cor_profile_interp));
    for j = 1:length(datasetSeparations)-1
        ...
        for i = datasetSeparations(j)+1:datasetSeparations(j+1)
            ...
            %           %% this isn't general yet!! make it works for all. e.g. where for
            %           % interval = 5 it's 0, for interval = 2 it's 400.
            %
            %           %                   ref_profile_shift(i,:)=NaN(1,size(ref_profile_interp,2));
            %           %                   cor_profile_shift(i,:)=NaN(1,size(cor_profile_interp,2));
            %
            % %           experiment with the right shift.
            %           timepoints_to_shift = 1000*(interval/maxInterval)
            %
            %           ref_profile_shift(i,timepoints_to_shift+1:end)= ref_profile_interp(i,1:end-timepoints_to_shift);
            %           cor_profile_shift(i,timepoints_to_shift+1:end)= cor_profile_interp(i,1:end-timepoints_to_shift);
            % %           ref_profile_shift(i,400+1:end)= ref_profile_interp(i,1:end-400);
            % %           cor_profile_shift(i,400+1:end)= cor_profile_interp(i,1:end-400);
            %
            %       else
            %           ref_profile_shift(i,:)= ref_profile_interp(i,:);
            %           cor_profile_shift(i,:)= cor_profile_interp(i,:);
            %       end
            % not sure if the shift below will work with a different max nb time
            % points (tp_max). maaybe the formula is:
            % (tp_max-1)*(maxInterval-interval)
%             timepoints_to_shift = 150*(maxInterval-interval);
            timepoints_to_shift = (tp_max-1)*(maxInterval-interval);
            ref_profile_shift(i,timepoints_to_shift+1:end)= ref_profile_interp(i,1:end-timepoints_to_shift);
            cor_profile_shift(i,timepoints_to_shift+1:end)= cor_profile_interp(i,1:end-timepoints_to_shift);
    ...
    % for i = betweenDatasets(j-1):betweenDatasets(j)
    %     interval=intervals(end);
    %
    %       % rescale with interpolation to 1 s intervals.
    %      ref_profile_interp(i,:)=interp1(interval:interval:tp_max*interval*2,ref_profile(i,:),1:tp_max*interval*2)
    %      cor_profile_interp(i,:)=interp1(interval:interval:tp_max*interval*2,cor_profile(i,:),1:tp_max*interval*2)
    %
    % end
    %remaining datasets
    %  if length(intervals)>1
    %  for j=1:length(betweenDatasets)
    %
    %      betweenDatasets(j)
    %      intervals;
    %  end
    ...
    % substitue NaNs with zero
    cor_profile_zero = cor_profile_shift;
    cor_profile_zero(isnan(cor_profile_zero)) = 0;
    ref_profile_zero = ref_profile_shift;
    ...
    %  plot(1:length(cor_profile_interp),cor_profile_interp);
    % plot at one-second intervals:
    subplot(5,2,[1,3]);plot(-(tp_max*maxInterval-1):tp_max*maxInterval, cor_profile_shift);
    %  subplot(5,2,[1,3]);plot(-(tp_max*maxInterval-1):tp_max*maxInterval, cor_profile_interp);
    
    axis([(axis_min-tp_max)*maxInterval, (axis_max-tp_max)*maxInterval,  min(min(cor_profile_shift)), max(max(cor_profile_shift))]);
    title('cor')
    subplot(5,2,[2,4]);plot(-(tp_max*maxInterval-1):tp_max*maxInterval, ref_profile_shift);
    axis([(axis_min-tp_max), (axis_max-tp_max),  min(min(ref_profile_shift)), max(max(ref_profile_shift))]);
    title('ref')
    
    subplot(5,2,[7,9])
    
    %  x = -(tp_max):tp_max;
    x = -(tp_max*maxInterval-1):tp_max*maxInterval;
    ...
    [hl,hp] = boundedline(x, y1, b(:,1,1), x, y2, b(:,1,2), 'alpha', 'cmap', [0 0.5 0; 1 0 1]);
    
    hl(1).LineWidth=1.5;
    hl(2).LineWidth=1.5;
    
    axis([(axis_min-tp_max)*maxInterval, (axis_max-tp_max)*maxInterval, intensity_min, intensity_max*1.5]);
    %  legend('cor', 'ref')
    xlabel('Time (sec)')
    ...
    % normalize and plot normalized
    
    % normalize
    cor_profile_norm = NaN(size(cor_profile_zero));
    ref_profile_norm = NaN(size(ref_profile_zero));
    for i = 1: size(cor_profile_zero,1)
        cor_profile_norm(i,:)=cor_profile_zero(i,:)/max(cor_profile_zero(i,:));
        ref_profile_norm(i,:)=ref_profile_zero(i,:)/max(ref_profile_zero(i,:));
    end
    ...
    y1_norm = nanmean(cor_profile_norm); %./2; % MA Temporary!!
    y2_norm = nanmean(ref_profile_norm);
    
    b_norm = zeros(size(y1, 2), 1, 2);
    b_norm(:, 1, 1) = nanstd(cor_profile_norm); %./2; % MA Temporary!!
    b_norm(:, 1, 2) = nanstd(ref_profile_norm);
    ...
    if saveIntensities
        ...
        %     ref_intensity_aligned = ref_aligned2cormax;
        %     cor_intensity_aligned = cor_aligned2cormax;
        % calculate mean std intensities aligned
        
        % make a table with: mean std intensities absolute,
        
        headers = {'ref_intensity_mean','ref_intensity_std','cor_intensity_mean','cor_intensity_std','ref_intensity_norm_mean','ref_intensity_norm_std','cor_intensity_norm_mean','cor_intensity_norm_std'};
        
        intensitiesTable = table(y2', b(:, 1, 2), y1', b(:, 1, 1), y2_norm', b_norm(:,1,2), y1_norm', b_norm(:,1,1), 'VariableNames', headers);
        
        % save. Note that this won't tell you how it was aligned, so be
        % consistent!
        
        writetable(intensitiesTable, 'intensities_aligned.csv', 'Delimiter', ',')
        
        csvwrite('intensities_aligned_cor_all.csv', cor_profile_zero')
    ...
    %  boundedline(x, [y1_norm, ; y2_norm], b_norm, 'g', 'm', 'alpha');
    [hl_norm, hp_norm] = boundedline(x, y1_norm, b_norm(:,1,1), x, y2_norm, b_norm(:,1,2), 'alpha', 'cmap', [0 0.5 0; 1 0 1]);
    axis([(axis_min-tp_max)*maxInterval, (axis_max-tp_max)*maxInterval, 0, 1]);
    %  legend('cor', 'ref')
    xlabel('Time (sec)')
    ...
    hl_norm(1).LineWidth=1.5;
    hl_norm(2).LineWidth=1.5;
    % or:
    
    % make the normalized plot instead for now
    makeCohorts=0;
