...
function [ref_assoc_persistant_nb, cor_assoc_persistant_nb, complete_associated_nb,complete_associated_nb_woHighLow,complete_associated_nb_woPersistent,...
        end_ref_unassociated_nb,begin_ref_unassociated_nb,end_cor_unassociated_nb,begin_cor_unassociated_nb,begin_associated_nb,...
        end_associated_nb,complete_unassociated_ref,complete_unassociated_cor] = Associate_tracks_20151221(data)
%% Parameter to use
...
%Read out the number of unassociated tracks starting at the beginning and
%end of the movie, CK 180803


end_ref_unassociated_nb =numel(track_stat_ref.ref_tracks_unassociated_high_breach);
begin_ref_unassociated_nb = numel(track_stat_ref.ref_tracks_unassociated_low_breach);

end_cor_unassociated_nb = numel(track_stat_cor.cor_tracks_unassociated_high_breach);
begin_cor_unassociated_nb = numel(track_stat_cor.cor_tracks_unassociated_low_breach);
...
for i=1:size(track_stat_ref.ref_tracks_associated)
    ...
    % change by CKaplan 180802, reference to the wrong variable
...
% commented line  987 and 988 out , CKaplan 180802 ; do not know were variable:
% num_thrown_tsrrta comes from but it basically gives the same number as num_thrown_ref_associate
...
%% moved this part out of the for loop , see line 928, Ckaplan 180802
 ref_unassoc_persistant_nb = numel(ref_unassoc_persistant_tracks);
    ref_assoc_persistant_nb = numel(ref_assoc_persistant_tracks);
    cor_unassoc_persistant_nb = numel(cor_unassoc_persistant_tracks);
    cor_assoc_persistant_nb = numel(cor_assoc_persistant_tracks);
    ...
    num_thrown_ref_associated = highindexa + lowindexa - numel(ref_assoc_persistant_tracks);
    num_thrown_cor_associated = highindexb + lowindexb - numel(cor_unassoc_persistant_tracks);
    num_thrown_ref_associated_addRefPersistent = highindexa + lowindexa + numel(ref_assoc_persistant_tracks) ;
    num_thrown_ref_associated_woHighLow = highindexa + lowindexa
% change CK 180802
% line 987 created a new track_stat_ref.ref_tracks_associated variable, see
% code; it seems that highindexa and lowindexa contain the number of
% persistent tracks.... see how num_thrown_ref_associated is calculated

complete_associated_nb = numel(track_stat_ref.ref_tracks_associated) - num_thrown_ref_associated
complete_associated_nb_woHighLow = numel(track_stat_ref.ref_tracks_associated) - num_thrown_ref_associated_woHighLow
complete_associated_nb_woPersistent = numel(track_stat_ref.ref_tracks_associated) -  num_thrown_ref_associated_addRefPersistent
begin_associated_nb = highindexa
end_associated_nb = lowindexa
complete_unassociated_ref = numel(track_stat_ref.ref_tracks_unassociated) - numel(track_stat_ref.ref_tracks_unassociated_high_breach) - numel(track_stat_ref.ref_tracks_unassociated_low_breach) -ref_unassoc_persistant_nb
complete_unassociated_cor = numel(track_stat_ref.ref_tracks_unassociated) - numel(track_stat_cor.cor_tracks_unassociated_high_breach) - numel(track_stat_cor.cor_tracks_unassociated_low_breach) -cor_unassoc_persistant_nb
