...
void Display3::displayFiber(Fiber const& fib)
{
    ...
    else if ( disp->line_style == 2 )
    {
        ...
        for ( unsigned int ii = 0; ii < fib.nbSegments(); ++ii )
        {
            ...
            // try to plot tension directly, can adjust rainbow or adjust here. alphaf?
...
//            gleHueColor(atan2(sqrt(d.XX*d.XX+d.YY*d.YY), d.ZZ) / ( 2 * M_PI ), 1.0); // phi
            gleHueColor(sqrt(d.XX*d.XX+d.YY*d.YY+d.ZZ*d.ZZ)); // r. scale by anything?
