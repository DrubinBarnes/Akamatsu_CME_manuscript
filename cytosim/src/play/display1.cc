...
void Display1::displayFiber(Fiber const& fib)
{
    ...
    else if ( disp->line_style == 2 )
    {
        ...
        for ( unsigned int ii = 0; ii < fib.lastPoint(); ++ii )
        {
            ...
            gleHueColor(1-fib.tension(ii)*1, 1.0);
            //gleJetColor(fib.tension(ii)*disp->rainbow, col.alphaf());
            gleVertex( fib.posPoint(ii) );
    ...
    else if ( disp->line_style == 3 )
    {
        ...
        for ( unsigned int ii = 0; ii < fib.lastPoint(); ++ii )
        {
            ...
            //gleHueColor(atan2(d.XX, d.YY) / ( 2 * M_PI ), 1.0);

           // gleHueColor(atan2(sqrt(d.XX*d.XX+d.YY*d.YY), d.ZZ) / ( 2 * M_PI ), 1.0); //try to make it r
              gleHueColor(atan2(sqrt(d.XX*d.XX+d.YY*d.YY), d.ZZ) / ( 10 * M_PI / 7 ), 1.0); // phi *7/10 of HUE color wheel blue to red

           // gleHueColor(sqrt(d.XX*d.XX+d.YY*d.YY+d.ZZ*d.ZZ), 1.0); // r. scale by anything?
           // gleHueColor(atan2(d.YY, d.XX)  / ( 2 * M_PI ), 1.0); // theta
