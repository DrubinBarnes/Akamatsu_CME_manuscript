                       ...
                       const real weight)
{
     ...
     */
    Vector axis = vecProd(db, da).normalized( sinus > 0 ? 1 : -1 );
    
    // rotate vector `db` around `arm` by angle specified as (cosinus, sinus):
    Vector rot = cosinus * db + sinus * vecProd(axis, db);
    
#elif ( DIM == 2 )
    
    // this correspond to the Z-direction, up or down:
    real dir = vecProd(da, db) > 0 ? 1 : -1;
    ...
    Vector fa = vecProd(Tq * na, da);
    Vector fb = vecProd(db, Tq * nb);
