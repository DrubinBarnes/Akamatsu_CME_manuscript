...
/**
 ...
 'fiber:singleClusters' | Clusters made of fibers connected by singles?
...
/* Copy of working fork reporting "Bridge"
...
void Simul::reportCoupleBridge(std::ostream& out) const
{
    for ( Couple * obj=couples.firstAA(); obj ; obj = obj->next() )\
    {
        ...
        real c = obj->hand1()->dir() * obj->hand2()->dir();
        out << " " << std::setw(9) <<  c;
        out << " " << std::setw(9) << obj->hand1()->dir();
        out << " " << std::setw(9) << obj->hand2()->dir();
        out << std::endl;
    }
}
...
void write(std::ostream& out, Single * obj, const char str[])
{
    ...
    if ( obj->attached() )
        out << "  " << std::setw(8) << obj->force() << "  " << std::setw(8) << obj->fiber()->number() << "  " << std::setw(8) << obj->dirFiber() << "  " << obj->hand()->abscissa(); // added this;
    else
        out << str << "0 0 0 0 0 0 0 0";
//        out << "  " << std::setw(8) << Vector(0,0,0) <<  "  " << std::setw(8) << real(0) << "  " << std::setw(8) << Vector(0,0,0) << "  " << real(0);
    out << std::endl;
}

//            out << "  " << std::setw(8) << obj->force() << "  " << std::setw(8) << obj->fiber()->number() << "  " << std::setw(8) << obj->dirFiber() << obj->abscissa; // added this;
//        out << "  " << std::setw(8) << obj->number();
...
{
    out << "% class id state position force fiber direction" << std::endl;
...
void Simul::reportSinglePosition(std::ostream& out, std::string const& who) const
{
    ...
    out << "% class id state position force fiber direction abscissa abscissa_plusEnd dirFromPlusEnd" << std::endl;
...
//void Simul::analyzeSingleClusters() const
//{
//    // set unique fleck() for each fiber:
//    for ( Fiber * fib = fibers.first(); fib; fib=fib->next() )
//        fib->fleck(fib->number());
//
//    // equalize fleck() when fibers are connected by a Couple:
//    for ( Single * cx = singles.firstA(); cx ; cx=cx->next() )
//    {
//        if ( cx->fiber()->fleck() != cx->fiber2()->fleck() )
//            reFleck(fibers, cx->fiber1()->fleck(), cx->fiber2()->fleck());
//    }
//}
